...
# Tambahkan path ke folder sebelumnya untuk import config & sensor_fusion
sys.path.append(os.path.join(os.path.dirname(__file__), '..', '05_pipeline_experiment'))

from config import OUT_FOLDER, CSV_FOLDER, WINDOW_SIZE_S, TARGET_HZ, get_logger
from sensor_fusion import apply_sensor_fusion

logger = get_logger(__name__)

# Direktori Output untuk 1D-CNN
CNN_DATA_DIR = os.path.join(os.path.dirname(__file__), "data")
...
def main():
    ...
    np.save(os.path.join(CNN_DATA_DIR, "X.npy"), X)
    np.save(os.path.join(CNN_DATA_DIR, "y.npy"), y)
    np.save(os.path.join(CNN_DATA_DIR, "groups.npy"), groups)
