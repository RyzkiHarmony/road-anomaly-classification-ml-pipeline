...
class TestScoreEvents:
    ...
    def test_empty_input_returns_empty(self):
        ...
        empty = pd.DataFrame(
            columns=["peak_vertical_g", "peak_gyro_mag", "vert_jrk", "event_duration"]
        )
    ...
    def test_score_is_between_0_and_1(self):
        ...
        df = _make_events(
            peak_vertical_g=[
                SCORE_ACCEL_MIN_G,
                (SCORE_ACCEL_MIN_G + SCORE_ACCEL_MAX_G) / 2,
                SCORE_ACCEL_MAX_G,
                SCORE_ACCEL_MAX_G + 5.0,
            ],
    ...
    def test_priority_levels_are_consistent_with_thresholds(self):
        ...
        for _, row in result.iterrows():
            if row["priority"] == "high":
                ...
                assert (
                    PRIORITY_MEDIUM_THRESHOLD <= row["score"] < PRIORITY_HIGH_THRESHOLD
                )
            else:
    ...
    def test_higher_accel_gives_higher_score(self):
        ...
        low_accel = _make_events(
            peak_vertical_g=[SCORE_ACCEL_MIN_G + 0.1],
            peak_gyro_mag=[0.0],
            vert_jrk=[0.0],
            event_duration=[0.0],
        )
        high_accel = _make_events(
            peak_vertical_g=[SCORE_ACCEL_MAX_G - 0.1],
            peak_gyro_mag=[0.0],
            vert_jrk=[0.0],
            event_duration=[0.0],
        )
