...
def benchmark_onnx_model(model_path, dummy_input_shape, num_iterations=1000):
    ...
    if dummy_input_shape is not None and len(dummy_input_shape) == len(resolved_shape):
        for i in range(len(raw_shape)):
            if raw_shape[i] is None or isinstance(raw_shape[i], str):
                resolved_shape[i] = dummy_input_shape[i]
