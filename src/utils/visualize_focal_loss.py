...
def main():
    ...
    plt.plot(p, loss_ce, 'k--', linewidth=2, label=r'Cross-Entropy ($\gamma=0$)')
    plt.plot(p, loss_fl_1, 'b-', linewidth=2, label=r'Focal Loss ($\gamma=1$)')
    plt.plot(p, loss_fl_2, 'g-', linewidth=2, label=r'Focal Loss Baseline ($\gamma=2.0$)')
    plt.plot(p, loss_fl_opt, 'r-', linewidth=2, label=r'Focal Loss Optuna ($\gamma=2.09$)')
