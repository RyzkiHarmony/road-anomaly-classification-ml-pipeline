...
def create_map(trip_id, df, events, out_path, csv_path):
    ...
    if meta and "startTime" in meta:
        ...
        if abs(csv_start_s - meta_start_s) < 86400 * 365: # Jika selisih < 1 tahun, berarti timebase sama
            ...
            print("    [INFO] Menggunakan startTime dari JSON untuk sinkronisasi detik audio.")
        else:
            print("    [WARN] Timebase CSV dan JSON berbeda. Menggunakan event pertama sebagai Detik 0.")
    else:
        print("    [WARN] Metadata JSON tidak ditemukan. Menggunakan event pertama sebagai Detik 0.")
