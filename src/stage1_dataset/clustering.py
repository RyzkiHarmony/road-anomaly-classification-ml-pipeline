...
_PREPROC_DIR = os.path.join(_SRC_DIR, "stage2_preprocessing")
if _PREPROC_DIR not in sys.path:
    sys.path.append(_PREPROC_DIR)
_FEAT_DIR = os.path.join(_SRC_DIR, "stage3_feature_engineering")
if _FEAT_DIR not in sys.path:
    sys.path.append(_FEAT_DIR)
...
def _cluster_to_event_row(cluster: dict, raw_df, raw_times) -> dict:
    ...
    if raw_df is not None and not raw_df.empty:
        ...
        mask = (times_raw >= time_dominant - (WINDOW_SIZE_S / 2.0)) & (times_raw <= time_dominant + (WINDOW_SIZE_S / 2.0))
        window_df = raw_df[mask]

        # Slice background for SNR calculation (e.g. -5s to -1s relative to event center)
        bg_mask = (times_raw >= time_dominant - 5.0) & (times_raw < time_dominant - 1.0)
        bg_df = raw_df[bg_mask]

        shape_features = extract_event_shape_features(window_df, bg_df)
    else:
        shape_features = extract_event_shape_features(None)
